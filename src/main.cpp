...
struct SensorHistory {
  ...
  static bool recoverReadPage(void *ctx, uint16_t const flashPageID) {
    SensorHistory *const self = static_cast<SensorHistory *>(ctx);
    SpiFlashOpResult const opResult = spi_flash_read(flashPageID << FlashPageExp, self->sliceL2Raw, FlashPageSize);
  ...
  static bool recoverIsErased(void *ctx) {
    return static_cast<SensorHistory *>(ctx)->erasedL2();
  }

  static bool recoverIsValid(void *ctx) {
    return static_cast<SensorHistory *>(ctx)->sliceL2.valid();
  }

  static uint64_t recoverUnixOffset(void *ctx) {
    return static_cast<SensorHistory *>(ctx)->sliceL2.unixOffset;
  }

  void recoverFlash() {
    RecoverFlashConfig const config = {
      .flashSectStart = FlashSectStart,
      .flashSectTotal = FlashSectTotal,
      .flashSectPageFactor = FlashSectPageFactor,
      .flashSectPageCount = FlashSectPageCount,
      .flashPageExp = FlashPageExp,
      .firstPageCountEmpty = 0,
    };
    RecoverFlashResult const result = recoverFlashLayout(
      config,
      this,
      recoverReadPage,
      recoverIsErased,
    ...
    headL2 = result.headL2;
    countL2 = result.countL2;
  }
